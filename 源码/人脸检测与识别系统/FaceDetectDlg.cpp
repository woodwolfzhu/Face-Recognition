...
BOOL CFaceDetectDlg::OnInitDialog()
{
	...
	EM=em1=em=ME=me1=me=ed1=ed=0.00;
...
void CFaceDetectDlg::LoadOriPixel(CDib *pDib)
{
	...
	method = new CSimilarHood(m_tOriPixelArray,m_nWndWidth,m_nWndHeight);
...
void CFaceDetectDlg::DoLOG(int left, int right, int top, int bottom, RGBQUAD **source, RGBQUAD **target)
{
	...
	for(j=0; j<m_nWndWidth; j++)
	{
		...
		{
			target[i][j].rgbRed = 255;  //脸部区域
		}
		else target[i][j].rgbRed = 0;//非人脸区域
	}
...
void CFaceDetectDlg::OnBtnBinary() 
{
	...
	for(int j=0; j<m_nWndWidth;  j++)
	{
		...
		m_tResPixelArray[i][j].rgbRed  = (int)(method->m_pBinaryArray[i][j]*255);
	}
	MakeBitMap();
	SetCursor(LoadCursor(NULL,IDC_ARROW));	
	MyDraw();
...
void CFaceDetectDlg::OnBtnHistogramH() 
{
	...
	for(int j=0; j<m_nWndWidth;  j++)	
	{
		...
		for(int i=0; i<m_nWndHeight; i++)
		{
			if(method->m_pBinaryArray[i][j] == 1) count++;
			m_tResPixelArray[i][j].rgbBlue = m_tResPixelArray[i][j].rgbGreen =	
...
// 计算相似度
/////////////////////////////////////////
void CFaceDetectDlg::OnBtnSimilarhood() 
{
	m_bShowFace = false;
	SetCursor(LoadCursor(NULL,IDC_WAIT));
	...
	{
		m_tResPixelArray[i][j].rgbBlue = m_tResPixelArray[i][j].rgbGreen =	
		m_tResPixelArray[i][j].rgbRed  = (int)(method->m_pLikeliHoodArray[i][j]*255);
	}
	MakeBitMap();
...
void CFaceDetectDlg::OnBtnMarkMouse() 
{
	...
	for(j=nLeft; j<=nRight; j++)
	{
		...
		if((R==G) && (G==B)) temp = 0;
		else temp = (double)(0.5*(2*R-G-B)/sqrt((double)(R-G)*(R-G)+(double)(R-B)*(G-B)));
		dlta = acos(temp);
		if(dlta < 0.2)
		{
			m_tResPixelArray[i][j].rgbRed = 255;  
...
void CFaceDetectDlg::OnBtnMatch() 
{
	...
	if(!m_bMatchOK)
	{
		AfxMessageBox("请先后打开两幅图像并分别进行所有操作后识别！");	
	}
	else
	{
